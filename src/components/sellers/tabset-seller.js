...
import { addSeller, addUser } from '../../actions';
import {
  IoIosAdd,
  IoIosTrash
} from 'react-icons/io';
...
/**
* @author
* @function Tabset_seller
**/

const Tabset_seller = (props) => {

    const [firstName, setFirstName] = useState('');
  const [lastName, setLastName] = useState('');
  const [email, setEmail] = useState('');
  const [show, setShow] = useState(false);
  const createSeller = useSelector(state => state.createSeller);
